...
tab1, tab2, tab3, tab4 = st.tabs(["Cyber Security","IT Security", "Passwords", "Data Health Check"])
...
with tab3:
    ...
    with st.expander("Try to break through your code (Verification)."):
        ...
        st.write("#")
        st.write("#")       
        st.write("* Find a full example on how to hack a python web application coded in Flask at: https://sqreen.github.io/DevelopersSecurityBestPractices/timing-attack/python")
 
with tab4:
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Toolbox Data Health Check:")
        check_credentials = st.checkbox('Check credential leakage with: https://haveibeenpwned.com/')
        check_pw = st.checkbox('Check new passwords against password lists: https://password.kaspersky.com/')
        if check_pw:
            st.write("Many password managers have this kind of check built in.")
        secret_check = st.checkbox("Check for leaked secrets: https://www.gitguardian.com/hasmysecretleaked")
