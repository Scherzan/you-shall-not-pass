...
tab0, tab1, tab2, tab3, tab4 = st.tabs(
    [
        "Where do I start?",
        "I have my code in a GitHub repo...",
        "How to check pip packages before installing?",
        "What to look for in dependency management?",
...
with tab1:
   ...
   """)
    ...
    st.button(
        "show Github actions example", on_click=switch_click, args=["github_action"]
    )
    ...
    if st.session_state["github_action"]:
        ...
        st.button(
            "Hide Github actions example", on_click=switch_click, args=["github_action"]
        )
...
with tab2:
    st.markdown(
        "### I googeled how to ... and the solution suggests to pip install some package."
    )
    col1, col2 = st.columns(2)
    with col1:
        caution = st.checkbox(
            "Be cautious, visit the page and check for a git repository."
        )
        if caution:
            auditing = st.checkbox(
                "Check for high risk python expressions such as subprocess.run, exec and eval functions."
            )
            if auditing:
                init_check = st.checkbox(
                    "Check common places for malicious code in __init__.py file or setup.py."
                )
                if init_check:
                    check_for_http = st.checkbox(
                        "Check for http requests exfiltrating system information."
                    )
                    with col2:
                        if check_for_http:
    ...
    col1, col2 = st.columns(2)
    with col1:
        check_vulnerability_db = st.checkbox(
            "Check databse for reports on the package: i.e. snyk database."
        )
        if check_vulnerability_db:
            st.write("https://security.snyk.io/vuln/pip")
            with col2:
                st.image("./pages/assets/search_snyk.png")
            automated_pypi_check = st.checkbox(
                "Identify malicious PyPI packages before download with guardDog."
            )
            if automated_pypi_check:
                st.write("guardDog: https://github.com/DataDog/guarddog")
                download_check = st.checkbox(
                    "Download latest package version from pypi and run code checks on code base."
                )
                if download_check:
                    st.write("Local code scan i.e. with Safety.")

    st.markdown("### I want to try out risky packages.")
    use_phylium = st.checkbox(
        "Use a sandbox environment i.e. phylum CLI with poetry support."
    )
    if use_phylium:
        st.write(
            "Sanbox package installations: https://blog.phylum.io/sandboxing-package-installations-arms-developers-with-defense-against-open-source-attacks-and-unintended-consequences/"
        )
...
with tab3:
    ...
    col1, col2 = st.columns(2)
    with col1:
        dep_management = st.checkbox(
            "Make sure you have repeatable and deterministic installations."
        )
        if dep_management:
            lockfiles = st.checkbox(
                "Use Lockfiles with pinned package versions, hashes and the full dependency tree.)",
                value=True,
            )
    ...
    scan_dep = st.checkbox(
        "Use a tool like pip-audit to scan your dependencies before going public."
    )
    if scan_dep:
        st.image("./pages/assets/pip_audit.png")

    advanced = st.checkbox(
        "Advanced: Using your own pypi repo locally (more a company level solution)."
    )
...
with tab4:
    ...
    with col1:
        ...
        st.write(
            "Semgrep Playground for code security heuristics: https://semgrep.dev/playground/new"
        )
        st.write(
            "Pip dependency management tools: https://pip-tools.readthedocs.io/en/stable/"
        )
        # st.write("Scan for leaked secrets: https://github.com/blacklanternsecurity/badsecrets (we did not test this)")
        st.subheader("Databases")
        st.write(
            "https://www.cve.org/ Official vulnerability Database (Search for Python)"
        )
        st.write(
            "https://security.snyk.io/ Based on CVE database with extended information on Poc and Fixes"
        )
        st.subheader("Get involved:")
        st.write(
            "Get startet with ethical hacking: https://snyk.io/ethical-hacking-resources/"
        )
        st.write("Python Software Foundation. https://www.python.org/psf/get-involved/")
        st.subheader("Organisations & Ressources")
        st.write(
            "https://cheatsheetseries.owasp.org/ Collection of specific application security topics"
        )
        st.write(
            "https://www.python.org/dev/security/ Reporting Security Issues with PyPi"
        )
        st.write(
            "https://pyfound.blogspot.com/2024/04/new-open-initiative-for-cybersecurity.html Python Software Foundation Open Source Security Efforts"
        )
    ...
    with col2:
        ...
        st.subheader("Blogs")
        st.write(
            "The Python Package Index Blog: https://blog.pypi.org/ (Well curated news on PyPi specific issues)"
        )
        st.write("GitGuardian Blog: https://blog.gitguardian.com/")
        st.write(
            "Phylium Research Blog: https://blog.phylum.io/tag/research/ (Security Reasearch)"
        )
        st.write(
            "Snyk Blog: https://snyk.io/blog/ (Variety of  Articels also on AI topcs i.e. Using Copilot)"
        )
        st.subheader("Podcasts")
        st.write(
            "The Real Python Podcast: https://realpython.com/podcasts/rpp/114/ (Remember to check shownotes.)"
        )
